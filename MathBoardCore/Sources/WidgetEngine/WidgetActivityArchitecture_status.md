...
- [x] Redesign gear popover for teacher use: Reset/Edit buttons, save-to-library star with folder dropdown, Questions count, Requires Student Work toggle, and editable tag chip list. Tags and requiresStudentWork live on `WidgetObject` (placed instance) and persist via `LibraryStoredItem` when saved to library.
- [x] Replace Longest Streak/Bonus/Points score table rows with 1st Attempt, After Corrections, and Ego Score (shown after Submit). SCORE nav row now shows `score/totalCount`. Stars flash on Submit.
- [x] Add `WidgetGearConfiguration` and `WidgetLibraryFolderDescriptor` public structs to `WidgetContract.swift`. Thread `gearConfiguration` from `WidgetActivityRenderer` → MC/FITB views → `ActivityScoreGaugePanel` → `ScoreSummaryTable` → `WidgetScoreSheetPopover`.
- [x] Add `tags: [String]` and `requiresStudentWork: Bool` to `WidgetObject` and `LibraryStoredItem` with backward-compat decoders.
...
- [x] Remove open/closed endpoint button from number-line control bar; toggle open/closed by tapping an already-selected endpoint.
- [x] Endpoint dots are 20pt diameter: closed = solid filled, open = stroke-only ring (no white fill). Selection overlay circle removed; ray handles are the only selection indicator.
- [x] Number-line Check button now scores the actual graph against the answer (not the auto-selected choice). Incorrect: question locks, contextual feedback shown (from `incorrectFeedback` JSON field), Next enables, question appears in "Retry Missed." Retry Missed gate: questions show previous attempt, student presses Retry (graph resets to initial state, Check + Next appear), or Skip (no score penalty, recorded in skippedRetryQuestionIDs). Hints hidden during retry. Data: `numberCorrectFirstTry` / `numberCorrectAfterRetry` / `skippedRetryQuestionIDs` already tracked.
- [x] Fix Live Progress denominator: `pointsPossible` in MC and FITB runtime `scoreRecord(for:)` now uses `document.questions.count` instead of `attempts`. Firebase `publishLiveProgress` now uses `pointsPossible` as `attemptedCount`, so the teacher's Live Progress drawer shows `score/questionCount` matching the score table.
- [x] Stop live progress loop from republishing submitted widgets: `publishLiveProgress(forActiveWidgetIDs:activeWidgets:)` in `StudentAssignedLessonView` now skips any widget whose ID is in `localSubmittedWidgetIDs`. Submitted scores are immutable after `submitWidgetScore` publishes the final record.
- [x] Fix teacher widget state leaking into student devices (and widgets disappearing from student canvas): `CanvasObjectSnapshot.write(to:)` gained a `preserving: Set<String> = []` parameter — listed files are skipped for both deletion and overwrite. `SlidesView.publishTeacherObjectSnapshot(for:)` strips `widgets.json` before publishing so teacher runtime state never travels over Ably/Firebase. `applyTeacherObjectSnapshot(_:)` calls `write(to:preserving: ["widgets.json"])` so the student's local widget state is never touched by an incoming snapshot, including old Firebase snapshots that still embed a `widgets.json`. `mergeInitialTeacherObjectSnapshots` (init-time path) also uses `preserving: ["widgets.json"]` so reopening the app after the teacher moves a widget doesn't delete the student's in-progress state.
- [x] Fix Live Progress score doubling for number-line widgets: `combinedScoreRecord` now uses ONLY the interactive-parts score (replacing the base MC/FITB score entirely) when scorable number-line parts exist. Previously it ADDED the MC "Done" choice score on top of the number-line score — doubling both `pointsPossible` and `score` for every question in a "Done/Revise" number-line widget. Result: a 2-question widget now shows 0/2 in Live Progress instead of 0/4.
- [x] Fix post-reset Live Progress: `mergeScoreRecords` detects when on-disk `activityRuntimeState` is nil but the widget is still in `localSubmittedWidgetIDs` (submitted this session), clears the submission lock and stale cached score, and lets the 2-second loop re-publish the fresh 0/N state. Previously resetting after submit permanently blocked re-publishing.
- [x] Add student sign-out on lesson exit: `FirebaseClassroomSyncService.deleteLessonPresence` deletes the student's presence record from Firebase. `StudentAssignedLessonView.onDisappear` calls `removeLessonPresence()` so stale presence is cleared when a student exits the lesson view.
