...
## 2026-08-21 — Fraction cursor fixes

Two related bugs fixed in `insert(_:)` and `moveExpressionCursorRight()`:

**Bug 1 — Arrow-right exit from denominator**: After pressing `→` at the end of a fraction denominator, the next typed character was still going into the denominator. Fixed by:
- `moveExpressionCursorRight()` sets `fractionCursorExitedAt` when cursor first reaches `denominatorRange.upperBound`. A second `→` press in that state now advances the cursor normally (breaks the "stuck at end" loop).
- `insert(_:)` now checks `fractionCursorExitedAt == currentOffset` before inserting. Boundary operators (`+`, `-`, `*`, `=`, etc.) are inserted directly — they naturally terminate the denominator scan. Non-boundary characters (letters, digits, `(`) prepend `*` to seal the denominator before inserting, e.g. `1/3` + arrow right + `x` → `1/3*x`.

**Bug 2 — Operators exiting the denominator mid-edit**: While typing inside a denominator, pressing `+` or `-` was moving the operator outside the fraction. Fixed by auto-wrapping the denominator in `()` on first such keystroke:
- When `+` or `-` is typed and the cursor is inside an unwrapped denominator, a `(` is inserted at `denominatorRange.lowerBound` to wrap the denominator, then the operator is appended.
- `moveExpressionCursorRight()` detects a paren-wrapped but unclosed denominator (starts with `(`, doesn't end with `)`) and inserts the closing `)` before setting `fractionCursorExitedAt`. This ensures the expression string is well-formed and evaluates correctly, e.g. `1/x` + `+3` + `→` → `1/(x+3)`.
- Already-closed denominators (`1/(x+3)`) are handled correctly by the existing `fractionCursorExitedAt` path.

Xcode live diagnostics: clean (one pre-existing iOS 26 deprecation warning unrelated to this change).

**Bug 3 — Implicit-product denominator collapse (parabola vs. horizontal line)**: Expression `y = 1/(2x) · x` was graphed as a parabola instead of a horizontal line at y = 1/2. Root cause: `normalizeImplicitVariableProducts` converts `2x` → `2*x`, so an already-parenthesized denominator `(2x)` expanded to `(2*x)` correctly, but a raw unparenthesized denominator like `2x` in `1/2x` became `1/2*x` (i.e. `(1/2)*x`), collapsing the denominator group before multiply/division precedence was applied.

Fixed by adding a `parenthesizeFractionDenominators` pre-processing step in `GraphCalculatorExpressionResolver`, called immediately before `normalizeImplicitVariableProducts`:
- `parenthesizeFractionDenominators(in: String)` scans left-to-right for each `/`. For the denominator starting at the character after `/`, it walks forward (tracking paren depth) until a boundary character (`+`, `-`, `*`, `/`, `=`, `,`, `<`, `>`) or end-of-string. If the denominator region is not already fully wrapped in a matching `()` pair, `(` and `)` are inserted around it.
- `parenthesizeFractionDenominators(in: GraphCalculatorPlot)` dispatches to the String overload for each plot variant.
- `isDenomBoundary` includes `/` (unlike the editor's `isFractionBoundaryOperator`) to stop at chained division and prevent misreading `1/2/3` as `1/(2/3)`.
- Three call sites updated in `scalarVariableValues` and `resolveRow` (scalar and main plot).

Result: `1/(2x)*x` → after parenthesization → `1/(2x)*x` (unchanged, already wrapped) → after normalization → `1/(2*x)*x` → evaluates correctly to `1/2`.

Xcode live diagnostics: clean on `GraphCalculatorExpressionResolver.swift`.
